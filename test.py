...
# Use the Google font and color scheme
tk.Style().configure('TButton', font='Helvetica 11')
tk.Style().configure('TEntry', font='Helvetica 11')
tk.Style().configure('TLabel', font='Helvetica 11')
tk.Style().configure('TProgressbar', font='Helvetica 11')
window.configure(background='#F1F1F1')

# Add a label and text field for entering the stock symbol
stock_label = tk.Label(window, text='Stock Symbol:', background='#F1F1F1')
...
train_button = tk.Button(window, text='Train Model', background='#4285F4', foreground='#FFFFFF', activebackground='#3367D6', activeforeground='#FFFFFF', command=lambda: train_model(stock_entry.get()))
train_button.grid(row=1, column=0, columnspan=2)

# Add a label and progress bar for showing the training progress
progress_label = tk.Label(window, text='Training Progress:', background='#F1F1F1')
progress_label.grid(row=2, column=0)
progress_bar = tk.ttk.Progressbar(window, orient='horizontal', length=200, mode='determinate', style='TProgressbar')
...
save_button = tk.Button(window, text='Save Model', background='#4285F4', foreground='#FFFFFF', activebackground='#3367D6', activeforeground='#FFFFFF', command=lambda: model.save('my_trading_bot.h5'))
